...
class FormateTesseract(QThread):
    ...
    def __init__(self, glass, parent=None):
        ...
        FormateLogger.log("TESSERACT THREAD, Time READ neural network detector training file, " + str(end - start))
    ...
    def shoot(self, image=None):
        ...
        for (startX, startY, endX, endY) in boxes:
            ...
            self.render_scheduler.append(FormateRect(startX, startY, endX, endY, "text_to_detect", roi))
            FormateLogger.log("TESSERACT THREAD, Decode text from rect position [" + str(startX) + "_" + str(startY) + "_" + str(endX) + "_" + str(endY) + "],0")

        return self.render_scheduler.render_scheduler
    ...
    def get_text_at_position(self, roi):
        ...
        FormateLogger.log("TESSERACT THREAD, Decode text from rect " + text + "," + str(end - start))
        return text

    def run(self):
        #print("TESSERACT THREAD: Tesseract Qthread running...")

        while True:
            while len(self.render_scheduler.screenshots) > 0:
                s = self.render_scheduler.screenshots.pop(0)
                #print("TESSERACT THREAD: Pop screenshot for processing by Tesseract: " + s.text)
                start = time.time()
                found_text_in_chop = self.shoot(s.im)
                end = time.time()
                FormateLogger.log("TESSERACT THREAD, execute shoot found text ," + str(end - start))
                #print("TESSERACT THREAD: Found the texts: " + str(found_text_in_chop))
